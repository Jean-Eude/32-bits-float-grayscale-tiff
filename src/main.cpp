...
def normalize_float32(data):
    data = data.astype(np.float32)  # Assure que c'est bien du float32
    min_val = np.min(data)
    max_val = np.max(data)

    if max_val == min_val:
        return np.zeros_like(data, dtype=np.float32)  # image uniforme → tout à zéro
    else:
